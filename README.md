...
The **emulation-provable port (phases F0-F6) is complete and green** (Renode + Docker micro-ROS agent,
no hardware). The only remaining phase is **F7 -- bring-up on a real NUCLEO-F446RE**.

| Phase | What works | `make` target | Tag |
|---|---|---|---|
| F1 | host unit tests (kinematics/pid/odom + encoder/pwm/imu/control math) | `test-host` | `stm32cube-p1-host-test-tier` |
| F0 | links real `libmicroros` (hard-VFP, 64-bit-atomics shim, no libstdc++) | `build-fw` | `stm32cube-f0-link-proven` |
| F2 | boots in Renode; FreeRTOS + transmits the micro-ROS ping | `renode` | `stm32cube-fullfw-gui-free` |
| F3 | live `micro_ros_agent` XRCE session (no board) | `agent-roundtrip` | `stm32cube-p4-agent-roundtrip` |
| F4 | encoder (TIM) + PWM motor + 50 Hz control loop + odometry | `control` | `stm32cube-p5-control` |
| F5 | MPU6050 IMU over real HAL I2C (Python mock slave in Renode) | `imu` | `stm32cube-p6-imu` |
| F6 | full base node: `/cmd_vel` + `/odom/unfiltered` + `/imu/data_raw` + reconnect | `topics` | `stm32cube-p7-topics` |
| F7 | **on real NUCLEO-F446RE hardware** | -- | *pending* |

What F7 still needs (things emulation cannot prove): real x4 quadrature counting & PWM duty, the
...
make renode       # F2: boots + transmits the ping
make control      # F4: control loop + injected-encoder -> odometry
make imu          # F5: MPU6050 over HAL I2C via a Python mock

# Tier C+ -- with a live micro-ROS agent (renode + docker + socat):
make agent-roundtrip   # F3: XRCE session
make topics            # F6: full cmd_vel/odom/imu round-trip
```

CI: `.github/workflows/stm32-f446re.yml` runs Tiers A/B + F2/F4/F5 on every push and PR, and F6
...
docs/STM32CUBE_PORTING_PLAN.md # design spec (phases F0-F7)
