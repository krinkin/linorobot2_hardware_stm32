# STM32Cube Port -- Plan 7: Full Base Node -- Topics + Reconnect + CI (Phase F6)
...
> emulation-provable port (F0-F6); F7 is on-hardware bring-up.
>
...
**Result (2026-06-05):** F6 PASS -- all 9 checks: XRCE session + participant; `/cmd_vel` sub +
...
All prior tiers stay green (host doctest, F0 link 108.9 KB no-libstdc++, F2/F4/F5).
...
- `/imu/mag` skipped (FakeMAG). Node name kept `stm32_node` (preserves the F3 harness).
...
- **MED:** the F6 `cmd_vel` check only proved the *agent* published -> relabeled honestly (firmware delivery
...
- **LOW:** odom could publish a torn pose/velocity tuple -> pose+velocity captured as one coherent
  ...
  control_smoke. CI soft-gate TODO. F6 test robustness: fixed `sleep 16` -> discovery poll loop; dead
  `grep -qv` removed; `frame_id` grep anchored.

## CI (.github/workflows/stm32-f446re.yml)
- `stm32-f0-f2` (always-on): host tests already; + libmicroros + build-fw + F2 (robot) + **F4 `make control`
  + F5 `make imu`** (Renode portable + socat installed).
- `stm32-topics` (new, push-gated, `continue-on-error` until proven stable, with a removal TODO): libmicroros
  + build-fw + **F6 `make topics`** (Docker micro_ros_agent + `--network host`).
...
- I2C SCL bus-recovery bit-bang (Plan 6), real MAG + MPU9250, the cmd->PWM *value* assertion over the agent
  (needs host-side Renode symbol readback during the docker run). F7 = on-hardware NUCLEO-F446RE bring-up.

## Reproduce
```bash
make test-all        # Tier A + libmicroros + build-fw + F2 + F4 + F5
make topics          # F6 full base-node topic round-trip (docker agent)
```
