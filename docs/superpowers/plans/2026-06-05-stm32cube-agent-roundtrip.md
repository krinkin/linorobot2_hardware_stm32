# STM32Cube Port -- Plan 4: Live micro-ROS Agent Round-Trip (Phase F3)

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:executing-plans. This plan is recorded **as-built** -- F3 was closed empirically on 2026-06-05. Steps are kept as `- [x]` to document what was proven and how to reproduce it.
>
> **Prerequisite:** Plan 2 (firmware ELF + libmicroros + atomics) and Plan 3 (F2 boot smoke) done. Needs `renode`, `docker`, `socat`.
...
Plan 4 of 7. Plans 1-3 precede it (host tests; skeleton+libmicroros+atomics; F2 boot smoke). Plan 4 = **F3 (live round-trip)**. It does NOT add the DMA/IT transport variants, encoder/PWM (Plan 5), or IMU (Plan 6) -- the round-trip uses the simple blocking HAL-UART transport, which is sufficient to prove the session. Neither emulator is baud/timing-accurate, so wire-timing fidelity stays a hardware-only item.
...
**Boot-smoke false-PASS gap (also fixed):** a configASSERT spin is **not** `HardFault_Handler`, so the old "PC != HardFault" smoke PASSED the hang. The F2 smoke now proves the firmware is genuinely alive (below).
...
| `firmware_stm32/renode/agent_bridge.sh` | F3 round-trip harness (Renode raw socket + socat + Docker agent) | Author |
| `firmware_stm32/renode/boot_smoke.sh` | F2 upgraded: assert USART2 emits the ping (catches the configASSERT class) | Modified |
| `firmware_stm32/renode/boot_smoke.robot` | F2 (CI): assert scheduler ticked (`xTickCount>0`, `uxCurrentNumberOfTasks!=0`) | Modified |
...
### Task 1 -- Wait-for-agent loop + valid priority  [OK]
...
### Task 3 -- Strengthen the F2 smoke so it can't false-PASS this class  [OK]
...
### Task 4 -- Wire it up  [OK]
- [x] `make agent-roundtrip` runs the bridge. `make renode` runs the upgraded F2.
...
make agent-roundtrip   # Renode + socat + Docker agent; expect "F3 PASS"
make renode            # upgraded F2: expect USART2 emits the ping
```

## Carry-forward
- CI runs F2 (`boot_smoke.robot` via renode-test-action). F3 needs Docker + socat on the runner -- left as an opt-in/local gate for now (heavier: builds the agent image, runs Renode + a container).
...
- Plans 5-7 (encoder/PWM control, IMU over HAL I2C, full topic loop + CI) are now complete (tags
  `stm32cube-p5-control`/`p6-imu`/`p7-topics`); the only remaining phase is F7 on-hardware NUCLEO-F446RE bring-up.
