# STM32Cube Port — Plan 5: Encoder (TIM) + PWM Motor + Control Loop (Phase Ф4)
...
>
> **Prerequisite:** Plans 1–4 done (host tests, F0 link, Ф2 boot, Ф3 agent round-trip).
...
Flash / ~73.6 KB RAM, hard-float, **no libstdc++ pulled in**), Ф2 boot smoke (firmware transmits the
ping), Ф4 control smoke (scheduler + 50 Hz loop run; an **injected encoder count moves odometry** with
...
- **Control on its own FreeRTOS task** (not an rclc timer): so the base is governed and the deadman
  brakes even when micro-ROS is disconnected — and so Ф4 is testable without the agent.
...
- **F446RE map:** encoders M1=TIM2(32-bit) PA0/PA1 AF1, M2=TIM3 PA6/PA7 AF2 (M3=TIM4, M4=TIM1 reserved;
  TIM5 is the µs base). PWM = TIM8 CH1..4 on PC6..PC9 AF3. Direction GPIOs PC0..PC3. USART2 (PA2/PA3) and
...
- **Time bases:** `millis()` = FreeRTOS tick (deadman + odom dt, matches the Arduino reference); `micros()`
  = free-running 1 MHz **TIM5** (true µs for the encoder RPM dt; works in Renode and on HW, unlike DWT).
...
- **MED:** encoder RPM dt was 1 ms-quantized (`tick*1000`) vs the reference's true µs → added the 1 MHz
  TIM5 µs base; one timestamp threaded into all four `getRPM`.
- **MED:** the Renode injection wasn't load-bearing (the generic TIM free-runs) → the Ф4 test now freezes
...
- **LOW:** two motors re-ran `HAL_TIM_PWM_Init` on shared TIM8 (EGR=UG re-pulse) → init each timer base
  once; `pwm_timing` now clamps `bits∈[1,32]` and the `lround` divisor in double; removed the board-wide
  `TIMER_CLOCK_HZ` (per-timer derivation kills the APB-doubling trap).

## Deferred to Plan 7 (Ф6, full loop + topics + CI)
...
make renode          # Ф2
make control         # Ф4 (load-bearing injected-encoder)
```
