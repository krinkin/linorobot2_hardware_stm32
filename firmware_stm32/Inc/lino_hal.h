...
// One I2C bus: instance + SCL/SDA pins (same AF) + bus speed. Open-drain, AF.
struct I2cDesc {
    I2C_TypeDef*  i2c;
    uint32_t      af;
    GPIO_TypeDef* port_scl; uint16_t pin_scl;
    GPIO_TypeDef* port_sda; uint16_t pin_sda;
    uint32_t      clock_speed;     // Hz (100000 = Standard mode)
};
...
// Enable the bus clock for a TIM / GPIO port / I2C (dispatch on the instance).
