...
#   make renode       # Renode boot smoke (Ф2) of the linked firmware (needs renode)
#   make test-all     # everything above, in order
#
# Prereqs: git submodules initialised  ->  git submodule update --init --recursive
#          arm-none-eabi-gcc, GNU make, docker; renode for the `renode` target.
...
.PHONY: help test-host libmicroros build-fw renode test-all clean submodules
...
# --- Tier C: Renode boot smoke (Ф2) ---
renode:
	bash $(FW)/renode/boot_smoke.sh $(FW)/build/firmware_stm32.elf

# --- everything ---
test-all: test-host libmicroros build-fw renode
